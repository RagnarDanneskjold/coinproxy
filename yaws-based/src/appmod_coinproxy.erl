...
bitcoin_rpc_handler([{ip, IP}] = _State, {call, getinfo, _Value} = _Request, _Session) ->
    io:format("Request = ~p, IP = ~p~n", [_Request, IP]),

    %% lab
    {ok, {{_, 200, _}, _, ResultStream}} = httpc:request(post, {"http://" ++ ?Username ++ ":" ++ ?Password ++ "@" ++ ?BitcoindHost ++ ":8332/", [], "text/json", "{\"jsonrpc\": \"2.0\", \"id\":\"1\", \"method\": \"getinfo\", \"params\": []}"}, [], []),
